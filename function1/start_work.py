...
async def process_gifts_inventory(worker_phone):
    ...
    async with async_playwright() as p:
        ...
        try:
            print(f"🌐 [ИНВЕНТАРЬ] Запуск по записи для {clean_phone}...")
            # Твой путь
            await page.goto("https://web.telegram.org/a/")
            await asyncio.sleep(10) # Даем время на загрузку
            # --- СКРИНШОТ 1: Проверка входа ---
            await page.screenshot(path=f"/app/load_{clean_phone}.png")
            await page.get_by_role("button", name="Open menu").click()
            await page.get_by_role("menuitem", name="My Profile").click()
            await asyncio.sleep(2)
            # --- СКРИНШОТ 2: Проверка открытия профиля ---
            await page.screenshot(path=f"/app/profile_{clean_phone}.png")
            await page.get_by_text("Gifts").click()
            await asyncio.sleep(4)
            # ЦИКЛ ПОВТОРЕНИЯ (для 2 и более подарков)
            while True:
                # 1. Твой клик по канвасу по координатам
                gift_canvas = page.locator("#RightColumn canvas").first
                if await gift_canvas.is_visible(timeout=7000):
                    print(f"📦 Клик по подарку (x:71, y:58)...")
                    await gift_canvas.click(position={"x": 71, "y": 58})
                    await asyncio.sleep(4)
                    # --- СКРИНШОТ 3: Проверка открытия карточки подарка ---
                    await page.screenshot(path=f"/app/gift_open_{clean_phone}.png")
                    # 2. Поиск кнопки Convert (используем регулярку, чтобы сумма звезд не мешала)
                    ...
                    if await convert_btn.is_visible(timeout=5000):
                        ...
                        print(f"✅ Продажа подтверждена.")
                        await asyncio.sleep(5)
                        # 4. Твой Close окна баланса
                        await page.get_by_role("button", name="Close").click()
                        await asyncio.sleep(3)
                    else:
                        print("ℹ️ Кнопка Convert не найдена в открытой карточке.")
                        await page.keyboard.press("Escape")
                        break 
                else:
                    print(f"📭 Подарков больше нет.")
                    break
        except Exception as e:
            # --- СКРИНШОТ 4: Ошибка ---
            await page.screenshot(path=f"/app/error_{clean_phone}.png")
            print(f"❌ [ИНВЕНТАРЬ-ERR] {clean_phone}: {e}")
        finally:
            await context.close()
async def check_inventory_loop(w_client, w_id, w_phone):
    """
    РАБОЧИЙ РЕЖИМ: Проверка подарков и баланса раз в 12 часов.
    Окна: 00:00 - 12:00 и 12:00 - 00:00.
    """
    print(f"📦 [ЭКОНОМИКА] Модуль инвентаря активен для {w_id} (режим окон).")
    while True:
        now = datetime.now()
        ...
        async with async_session() as session:
            # Проверяем в БД, не отработал ли этот аккаунт уже в этом окне
            ...
            if not worker or worker.last_inventory_check_window_end == current_window_end:
                # Если уже проверяли — спим 10 минут и проверяем снова (наступило ли новое окно?)
                await asyncio.sleep(600)
                continue
            # 2. ВЫБИРАЕМ РАНДОМНЫЙ МОМЕНТ ВНУТРИ ОСТАВШЕГОСЯ ВРЕМЕНИ ОКНА
            seconds_remaining = (current_window_end - now).total_seconds()
            # Берем случайную паузу от 0 до конца окна
            random_wait = random.uniform(0, seconds_remaining)
            print(f"⏳ [ИНВЕНТАРЬ] Аккаунт {w_id} запланировал вход через {int(random_wait/60)} мин.")
            await asyncio.sleep(random_wait)
            # 3. ЗАПУСК ИСПОЛНЕНИЯ (Браузер -> API -> БД)
            try:
                # А) Продажа подарков
                await process_gifts_inventory(w_phone)
                # Б) Обновление баланса через API
                await update_stars_balance_single(w_client, w_id)
                # В) Фиксация окна в базе
                ...
                print(f"✅ [ЭКОНОМИКА] Аккаунт {w_id} успешно закрыл окно до {current_window_end}")
            except Exception as e:
                print(f"❌ [ЭКОНОМИКА-ERR] Сбой у {w_id}: {e}")
                # При ошибке спим 15 минут, чтобы не частить запросами
                await asyncio.sleep(900)
async def update_stars_balance_single(w_client, w_id):
    """
    Разовая проверка баланса через API.
    Вызывается строго внутри check_inventory_loop после завершения работы браузера.
    """
    print(f"💰 [ЭКОНОМИКА] Обновление баланса аккаунта {w_id} после продажи...")
    try:
        from telethon.tl.functions.payments import GetStarsStatusRequest
        # Запрашиваем актуальное состояние звезд у Telegram
        stars_status = await w_client(GetStarsStatusRequest(peer='me'))
        current_balance = int(stars_status.balance.amount) 
        # Проверяем, достаточно ли звезд для участия в платных активностях (порог 40)
        ...
        async with async_session() as session:
            await session.execute(
                ...
                .values(
                    ...
                    # Обновляем метку времени последней проверки баланса
                    last_check_window_end=func.now() 
                )
            )
            await session.commit()
        print(f"✅ [БАЛАНС] Аккаунт {w_id}: {current_balance} ⭐. Статус готовности: {is_ready}")
    except Exception as e:
        print(f"❌ [БАЛАНС-ERR] Не удалось обновить звезды для {w_id}: {e}")
...
import random
import re
from patchright.async_api import async_playwright # Используем твой Patchright
import base64
# В Docker/Linux путь обычно такой, если ставим через apt-get install tesseract-ocr
    ...
    async with async_playwright() as p:
        context = await p.chromium.launch_persistent_context(
            ...
            args=[
                ...
                "--disable-infobars", # Убирает плашку "Браузером управляет ПО"
                "--window-size=1280,720" # Задаем явный размер окна
                "--disable-gpu", # ОБЯЗАТЕЛЬНО ДЛЯ DOCKER
                "--disable-dev-shm-usage" # Помогает, если мало памяти в контейнере
            ]
        )
        ...
        try:
            ...
            await asyncio.sleep(5)
                                                # 4. УНИВЕРСАЛЬНЫЙ КЛИК (СТРОГО ПО КНОПКАМ ПОД ТЕКСТОМ)
            print(f"🔘 Ищем кнопки участия в посте #{post_id}...")
            
            try:
                # Находим контейнер кнопок ПОД сообщением. 
                # Это исключает системные кнопки "Share", "Reply" и т.д.
                buttons_container = page.locator(f"#message-{post_id} .reply-buttons")
                
                # Ждем, пока блок с кнопками под постом прогрузится
                await buttons_container.wait_for(state="visible", timeout=10000)

                # Берем ПЕРВУЮ кнопку внутри блока кнопок участия
                # Ищем по классу .reply-buttons-button, который есть у всех кнопок-ботов
                target_btn = buttons_container.locator(".reply-buttons-button").first
                
                if await target_btn.is_visible():
                    print(f"🎯 Жму кнопку участия в блоке .reply-buttons...")
                    await target_btn.click()
                    print("✅ Кнопка нажата успешно")
                else:
                    raise Exception("Кнопка участия не видна")

            except Exception as e:
                print(f"❌ Ошибка: {e}. Пробую фолбэк (первый div с ролью button)...")
                # Если контейнер не найден, жмем ПЕРВЫЙ div/button внутри поста, 
                # НО исключая те, что имеют класс "message-action-button" (это и есть твой Share)
                try:
                    fallback_btn = page.locator(f"#message-{post_id} .reply-buttons-button, #message-{post_id} button:not(.message-action-button)").first
                    await fallback_btn.click()
                    print("✅ Нажато через фолбэк (исключая Share)")
                except:
                    return False

                                   # 5. КНОПКА CONFIRM + ОЖИДАНИЕ MINI APP
            ...
            try:
                ...
                print("ℹ️ Нажат Enter")
            # --- ВОТ ТУТ УВЕЛИЧИВАЕМ ВРЕМЯ (Ждем прогрузки Mini App) ---
            print("⏳ Ждем 10 секунд для полной прогрузки Mini App...")
            await asyncio.sleep(10) 
            # --- БЛОК НАЖАТИЯ "Я НЕ РОБОТ" (ВНУТРИ IFRAME) ---
            try:
                # Mini App всегда в iframe, поэтому ищем кнопку там
                app_frame = page.frame_locator("iframe").first
                check_btn = app_frame.locator("#turnstile_check")
                # Ждем появления кнопки внутри фрейма еще до 10 сек
                await check_btn.wait_for(state="visible", timeout=10000)
                print("🔘 Нажимаю 'Я не робот' внутри Mini App...")
                await check_btn.click()
                # Еще чуть-чуть времени, чтобы подгрузилась картинка с цифрами
                await asyncio.sleep(5)
                print("✅ Кнопка нажата, ждем появления капчи")
            except Exception as e:
                print(f"⚠️ Кнопка 'Я не робот' не найдена во фрейме: {e}")
                            # --- БЛОК РЕШЕНИЯ КАПЧИ (ИСПРАВЛЕННЫЙ) ---
            print("🧩 Начинаю цикл распознавания...")
            app_frame = page.frame_locator("iframe").first
            while True:
                try:
                    # 1. СНАЧАЛА ПРОВЕРЯЕМ: может капча уже исчезла?
                    answer_input = app_frame.locator("#turnstile__answer")
                    # Если инпут не виден в течение 3 секунд — значит капча пройдена
                    if not await answer_input.is_visible(timeout=3000):
                        print("🎉 Капча исчезла! Похоже, мы успешно участвуем.")
                        break
                    # 2. Если инпут всё еще на месте, работаем с картинкой
                    captcha_img = app_frame.locator("img[alt='captcha']")
                    if not await captcha_img.is_visible():
                        print("✅ Картинка не найдена, выходим из цикла.")
                        break
                    ...
                    final_answer = digits[:5]
                    print(f"🤖 Ввожу ответ: {final_answer}")
                    # 3. Ввод и ОК
                    # Используем force=True, если вдруг элемент считается disabled в моменте
                    await answer_input.fill(final_answer, timeout=5000)
                    ok_button = app_frame.locator(".turnstile__answer_button")
                    await ok_button.click()
                    print("🚀 Нажал ОК, жду 5 сек на проверку...")
                    # Даем время сайту подумать и скрыть окно
                    await asyncio.sleep(5)
                except Exception as e:
                    # Если во время любой операции (fill/click) элемент исчез — это успех
                    print(f"ℹ️ Окно капчи закрылось во время работы (Успех)")
                    break
            print("✅ Работа с веб-капчей завершена успешно")
            await page.screenshot(path=f"/app/screenshots/final_success_{clean_phone}.png")
            return True
        except Exception as e:
...
async def run_worker_instance(w_data):
    ...
    try:
        ...
        tasks = [
            # 1. Менеджер подписок (входы/выходы)
            asyncio.create_task(subscription_manager_loop(w_client, w_id)),
            # 2. ЕДИНЫЙ МОДУЛЬ ЭКОНОМИКИ (Инвентарь + Баланс)
            # Оставляем ТОЛЬКО ЭТУ строку для инвентаря
            asyncio.create_task(check_inventory_loop(w_client, w_id, w_data.phone)),
            # 3. CRM-система (ЛС и ответы оператора)
            asyncio.create_task(worker_outgoing_loop(w_client, w_id)),
            # 4. Десант удачи (эмодзи под посты)
            ...
            # 9. Прогрев аккаунтов (переписка в группе)
