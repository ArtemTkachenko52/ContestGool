...
#     async with async_session() as session:
#         async with async_playwright() as p:
#             # Запускаем контекст с защитой от детектирования
#             context = await p.chromium.launch_persistent_context(
#                 user_data_dir, 
#                 headless=True, 
#                 args=[
...
#                 try:
#                     # Ищем любые кнопки OK, Close, Update, Cancel, Закрыть
#                     popups = page.get_by_role("button", name=re.compile(r"OK|Close|Update|Закрыть|Cancel", re.I))
#                     # Если нашли хоть одно — кликаем по первому
#                     if await popups.first.is_visible(timeout=5000):
#                         await popups.first.click()
#                         print(f"🔘 [SETUP] Окно-заглушка закрыто для {worker.phone}.")
#                         await asyncio.sleep(3)
#                 except:
...
#                         if random.random() < 0.3:
#                             try:
#                             # Прямой переход в настройки ДР
#                                 await page.get_by_role("button", name=re.compile(r"День рождения", re.I)).click()
...
#                             file_path = f"/app/assets/avatars/{avatar}"
#                             if os.path.exists(file_path):
#                                 async with page.expect_file_chooser() as fc_info:
#                                     await page.get_by_role("button", name=re.compile(r"фото", re.I)).first.click()
